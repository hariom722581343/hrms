...
String msg = ConsoleColors.RED_BOLD +"Department Insertion Unsuccessful...";
	...
	public String updateDept(int dept_id) {
        ...
        String msg = ConsoleColors.RED_BOLD +"Department Update Unsuccessful...";
		...
		String msg = ConsoleColors.RED_BOLD +"Leave Apply Unsuccessful...";
			...
			try(Connection con = DaoConnection.provideConnection()){
				...
				if(rs.next()) {
					  ...
					  Scanner sc = new Scanner(System.in);
					  System.out.println(ConsoleColors.YELLOW_UNDERLINED +"Enter Number Of Days For Leave");
					  String leave_day = sc.next();
					PreparedStatement ps1 = con.prepareStatement("insert into leaveapplication(leave_day,emp_id) values(?,?)");
	...
	public List<EmployeeLeave> viewEmpLeave() {
		...
		try(Connection con = DaoConnection.provideConnection()){
				...
				PreparedStatement ps1 = con.prepareStatement("select employee.emp_id, employee.emp_name,leaveApplication.leave_day,leaveApplication.status from employee inner join leaveApplication on employee.emp_id = leaveApplication.emp_id;");				
	...
	public String leavePermit( int emp_id, String Status) {
		...
		try(Connection con = DaoConnection.provideConnection()){
				...
				PreparedStatement ps1 = con.prepareStatement("update leaveApplication set status = ? where emp_id = ?  ");
	...
	public EmployeeLeave getLeaveStatusById(String pass) {
		...
		try(Connection con = DaoConnection.provideConnection()){
			...
			ResultSet rs = ps.executeQuery();
			if(rs.next()) {
				PreparedStatement ps1 = con.prepareStatement("select employee.emp_id, employee.emp_name, leaveApplication.leave_day,leaveApplication.status from employee inner join leaveApplication on employee.emp_id = leaveApplication.emp_id where employee.emp_id = ? ;");				
	...
	public int validateAdmin(String admin_name) {
		...
		try(Connection con = DaoConnection.provideConnection()){
			...
			ResultSet rs = ps.executeQuery();
			if(rs.next()) {			   
				    System.out.println(ConsoleColors.YELLOW_UNDERLINED +"Please Enter Your Password");
